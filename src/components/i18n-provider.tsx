...
export const I18nProvider = component$<I18nProviderProps>(({ initialLocale }) => {
  const location = useLocation();
  const locale = useSignal<SupportedLocale>(
  ...
  useTask$(({ track }) => {
    ...
    if (SUPPORTED_LOCALES.includes(potentialLocale)) {
      ...
      const browserLang = globalThis.navigator?.language?.split('-')[0] as SupportedLocale;
  ...
  const tp = $((key: string, count: number, params?: Record<string, any>) => {
    return getPluralTranslation(translations[locale.value], key, count, params);
  });
